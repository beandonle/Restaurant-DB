...
CREATE TABLE shift(
  ...
  startDate          TIMESTAMP,
  endDate            TIMESTAMP,
  CONSTRAINT shift_pk  PRIMARY KEY (shiftType, startDate, endDate)
);

CREATE TABLE job(
  jobPosition        VARCHAR(20) NOT NULL,
  jobTitle           VARCHAR(20) NOT NULL ,
  CONSTRAINT job_pk  PRIMARY KEY (jobPosition)
);
...
CREATE TABLE employee(
  ...
  CONSTRAINT employeee_fk2  FOREIGN KEY (jobPosition) REFERENCES job (jobPosition),
...
CREATE TABLE mentorship(
  ...
  endDate                     DATE,
  CONSTRAINT mentorship_pk    PRIMARY KEY (itemID, eID, sChefID, startDate),
  CONSTRAINT mentoryship_fk1  FOREIGN KEY (itemID) REFERENCES menuItem (itemID),
  CONSTRAINT mentoryship_fk2  FOREIGN KEY (eID) REFERENCES sousChef (eID),
  CONSTRAINT mentoryship_fk3  FOREIGN KEY (sChefID) REFERENCES sousChef (eID)
);
...
CREATE TABLE tableInfo(
  tableNum             VARCHAR(5) NOT NULL,
  eID                  VARCHAR(20) NOT NULL,
  tableCap             INT NOT NULL,
  reservationStat      VARCHAR(20),
  CONSTRAINT table_pk  PRIMARY KEY (tableNum),
  CONSTRAINT table_fk  FOREIGN KEY (eID) REFERENCES waitStaff (eID)
);
